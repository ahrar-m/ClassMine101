# Trigonometric Ratios & Particular Angles

!!! info "Chapter Overview (NCERT Ch 8 / RS Aggarwal Ch 10 & 11)"
    Complete theoretical foundation, geometric definitions, reciprocal & quotient identities, scale-independence proofs, and evaluation of trigonometric ratios for acute angles ($0^\circ, 30^\circ, 45^\circ, 60^\circ, 90^\circ$).

---

## Topics

<div class="grid cards" markdown>

-   :material-triangle-outline: **Introduction & Right Triangle Geometry**

    ---

    Definition of acute angle $\theta$, Hypotenuse, Perpendicular (Opposite side), and Base (Adjacent side) in a right-angled triangle.

    *:material-clock-outline: Coming soon*

-   :material-function-variant: **The Six Trigonometric Ratios**

    ---

    Definitions of $\sin \theta$, $\cos \theta$, $\tan \theta$, $\csc \theta$, $\sec \theta$, and $\cot \theta$.

    *:material-clock-outline: Coming soon*

-   :material-swap-horizontal: **Reciprocal & Quotient Relations**

    ---

    Fundamental relations: $\csc \theta = \frac{1}{\sin \theta}$, $\sec \theta = \frac{1}{\cos \theta}$, $\cot \theta = \frac{1}{\tan \theta}$, $\tan \theta = \frac{\sin \theta}{\cos \theta}$, $\cot \theta = \frac{\cos \theta}{\sin \theta}$.

    *:material-clock-outline: Coming soon*

-   :material-ruler-square: **Scale Independence of T-Ratios**

    ---

    Proof using similar triangles showing that T-ratios depend strictly on the angle $\theta$ and not on the size of the triangle.

    *:material-clock-outline: Coming soon*

-   :material-calculator: **Evaluating T-Ratios using Pythagoras Theorem ($k$-Method)**

    ---

    Given one trigonometric ratio, determine all remaining five ratios using the right triangle property and constant ratio factor $k$.

    *:material-clock-outline: Coming soon*

-   :material-angle-acute: **T-Ratios of Specific Angles ($0^\circ, 30^\circ, 45^\circ, 60^\circ, 90^\circ$)**

    ---

    Geometric derivations of ratio values for standard angles, the T-ratio values table, and evaluation of trigonometric expressions.

    *:material-clock-outline: Coming soon*

-   :material-book-open-outline: **NCERT Walkthroughs (Ex 8.1 & Ex 8.2)**

    ---

    Step-by-step solutions and explanations for all NCERT textbook problems.

    *:material-clock-outline: Coming soon*

-   :material-notebook-edit-outline: **RS Aggarwal High-Yield Practice Problems**

    ---

    Exhaustive practice set covering RS Aggarwal Chapter 10 & 11 problem types.

    *:material-clock-outline: Coming soon*

</div>
